...
This checkout is maintained as an ongoing fork with local features rebased onto newer upstream niri releases. Treat every new feature or behavior change in this fork as fork-local unless the user explicitly says otherwise; upstreamability is secondary to local usefulness and easy future rebases.

Every fork-local feature should include a matching `wf_features/*.md` document in the same feature sequence. Write these notes as behavior contracts, not implementation walkthroughs: explain the user-visible purpose, intended behavior, edge cases, public commands/configuration/IPC if any, and validation expectations. Avoid depending on current module names, helper functions, or rendering structure except where they are public API, so the feature can be reimplemented from the document even if the internals are replaced.

Any new fork-local feature should also include focused tests in the same feature commit whenever practical, so rebases fail loudly instead of silently dropping behavior. Prefer tests that lock the public contract, regression trigger, or pure logic behind the feature; if a full compositor/rendering integration test is impractical, add lower-level unit, serialization, or state tests and record any remaining manual validation.
